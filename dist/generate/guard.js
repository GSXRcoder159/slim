...
import { pathEscapesRoot } from "../rewrite/paths.js";
...
export class OriginalSourceGuard {
    static assertNotOriginalImpl(filePath) {
        ...
        if (IMPL_EXT.test(norm) || MAP_EXT.test(norm) || TEST_DIR.test(norm) || TEST_FILE.test(base)) {
            throw new SlimExit(EXIT_FAIL, `OriginalSourceGuard: refused to read ${filePath}`);
        }
    }
    static readPublicSpec(filePath) {
        if (!filePath.endsWith(".d.ts") && !/README(\.md)?$/i.test(filePath)) {
            throw new SlimExit(EXIT_FAIL, `OriginalSourceGuard: public spec must be .d.ts or README, got ${filePath}`);
        }
        return guardedReadFileSync(filePath);
    }
}
/** Refuse metadata that is absolute or that resolves outside `allowedRoot`. */
export function assertDeclaredSpecInside(allowedRoot, rel) {
...
export function assertPublicSpecInside(allowedRoot, candidate) {
...
export function guardedReadFileSync(filePath) {
    OriginalSourceGuard.assertNotOriginalImpl(filePath);
    return readFileSync(filePath, "utf8");
}
export function slimRoot() {
