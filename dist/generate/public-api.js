...
import { pathEscapesRoot, toPosixPath } from "../rewrite/paths.js";
import { EXIT_FAIL, SlimExit } from "../exit.js";
import { OriginalSourceGuard, assertDeclaredSpecInside, assertPublicSpecInside, } from "./guard.js";
const README_CAP = 8000;
export function loadPublicApi(projectRoot, pkg, subpath = "") {
    ...
    if (subpath) {
        const subRel = typesFromExports(meta?.exports, `./${subpath}`) ??
            typesFromExports(meta?.exports, subpath);
        const subAbs = firstDts(dir, subRel, join(dir, `${subpath}.d.ts`));
        if (subAbs)
            return specFromDts(projectRoot, subAbs, "subpath-dts");
    }
    const rootRel = typesFromExports(meta?.exports, ".") ??
        (typeof meta?.types === "string" ? meta.types : undefined) ??
        (typeof meta?.typings === "string" ? meta.typings : undefined);
    const bundled = firstDts(dir, rootRel, join(dir, "index.d.ts"), join(dir, `${bareName(pkg)}.d.ts`));
    if (bundled)
        return specFromDts(projectRoot, bundled, "bundled-dts");
    ...
    if (existsSync(typesDir)) {
        ...
        if (dt)
            return specFromDts(projectRoot, dt, "types-package");
    }
    ...
    const limitation = `package ${pkg} — no local .d.ts or README; implement from envelope call sites only. Do not invent undocumented overloads.`;
    return { text: limitation, source: "envelope-only", limitation };
}
function specFromDts(projectRoot, abs, source) {
    return {
...
function assertInsideNodeModules(projectRoot, nodeModules, dir) {
...
function packageDir(projectRoot, pkg) {
...
function typesFromExports(exportsField, key) {
    if (exportsField == null)
        return undefined;
    if (key === "." && typeof exportsField === "string")
        return dtsOnly(exportsField);
...
function typesFromCondition(value) {
    ...
    if (value && typeof value === "object") {
        const o = value;
        return (typesFromCondition(o.types) ??
 ...
 */
function firstDts(packageRoot, declaredRel, ...candidates) {
    if (declaredRel) {
...
function firstExisting(packageRoot, ...paths) {
