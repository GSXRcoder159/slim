...
export interface Moment {
  ...
  valueOf(): number;
  toDate(): Date;
  add(amount: number | Record<string, number>, unit?: string): Moment;
  subtract(amount: number | Record<string, number>, unit?: string): Moment;
  isValid(): boolean;
}
...
function parseInput(input?: Date | string | number | null): Date {
  if (input === undefined) return new Date(Date.now());
...
function tokenValue(date: Date, tok: string): string {
  switch (tok) {
    case "YYYY":
...
function applyUnit(date: Date, n: number, unit: string): void {
...
class SlimMoment implements Moment {
  ...
  add(amount: number | Record<string, number>, unit?: string): Moment {
    ...
    if (amount !== null && typeof amount === "object") {
      for (const [u, n] of Object.entries(amount)) {
        if (typeof n === "number") applyUnit(this.date, n, u);
      }
      return this;
    }
    if (typeof amount === "number" && unit) applyUnit(this.date, amount, unit);
    return this;
  }
  ...
  subtract(amount: number | Record<string, number>, unit?: string): Moment {
    if (amount !== null && typeof amount === "object") {
      ...
      for (const [u, n] of Object.entries(amount)) negated[u] = -n;
...
export function createMoment(input?: Date | string | number | null): Moment {
