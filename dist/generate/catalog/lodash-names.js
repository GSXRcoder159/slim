...
const BY_LOWER = new Map(LODASH_SYMBOLS.map((symbol) => [symbol.toLowerCase(), symbol]));
export function lodashNpmName(symbol) {
    return `lodash.${symbol.toLowerCase()}`;
}
/** Map `isempty` / `isEmpty` onto the registered symbol, or undefined if unsupported. */
export function canonicalLodashSymbol(suffix) {
    return BY_LOWER.get(suffix.toLowerCase());
}
//# sourceMappingURL=lodash-names.js.map