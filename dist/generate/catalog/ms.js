...
const PARSE = /^(-?(?:\d+)?\.?\d+) *(milliseconds?|msecs?|ms|seconds?|secs?|s|minutes?|mins?|m|hours?|hrs?|h|days?|d|weeks?|w|years?|yrs?|y)?$/i;
...
function plural(msVal, unit, name) {
    const abs = Math.abs(msVal);
    const n = Math.round(msVal / unit);
    return `${n} ${name}${abs >= unit * 1.5 ? "s" : ""}`;
}
function format(msVal, long) {
    const abs = Math.abs(msVal);
    if (long) {
        if (abs >= D)
            return plural(msVal, D, "day");
        if (abs >= H)
            return plural(msVal, H, "hour");
        if (abs >= M)
            return plural(msVal, M, "minute");
        if (abs >= S)
            return plural(msVal, S, "second");
        return `${msVal} ms`;
    }
    if (abs >= D)
        return `${Math.round(msVal / D)}d`;
    if (abs >= H)
        return `${Math.round(msVal / H)}h`;
    if (abs >= M)
        return `${Math.round(msVal / M)}m`;
    if (abs >= S)
        return `${Math.round(msVal / S)}s`;
    return `${msVal}ms`;
}
