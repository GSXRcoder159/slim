import { formatRefuse } from "../../scan/refuse.js";
import { catalogSymbols, canonicalCatalogPkg, matchCatalog } from "./index.js";
import { canonicalLodashSymbol } from "./lodash-names.js";
import { MIME_EXTENSION_TYPES, MIME_LOOKUP_EXTS } from "./mimeTypes.js";
...
export function catalogBoundary(env, pkg) {
    const family = canonicalCatalogPkg(pkg);
    if (!CATALOG_FAMILIES.has(family))
        return null;
    if (pkg.startsWith("lodash.")) {
        const suffix = pkg.slice("lodash.".length);
        if (!canonicalLodashSymbol(suffix)) {
            return tooWide(pkg, family, `package ${pkg}`, `blocking: ${pkg} is not a registered lodash.* alias`);
        }
    }
    ...
    if (family === "moment") {
        ...
        if (members.length) {
            return tooWide(pkg, family, `result members: ${resultMembers(env).join(", ") || "(none)"}`, `blocking: ${members.join(", ")}`);
        }
    }
...
export function formatCatalogRefuse(r) {
    return formatRefuse(r);
}
function tooWide(pkg, family, evidence, blocking) {
...
function shapeStrings(shape) {
    ...
    const out = [];
    if (shape.literals) {
        for (const v of shape.literals)
            if (typeof v === "string")
                out.push(v);
    }
    if (shape.elements) {
        for (const el of shape.elements)
            out.push(...shapeStrings(el));
    }
    if (shape.props) {
        for (const el of Object.values(shape.props))
            out.push(...shapeStrings(el));
    }
    return out;
}
