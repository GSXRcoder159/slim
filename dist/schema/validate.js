...
function classifySchemaVersion(schema, value, path) {
    const spec = schema.properties?.schemaVersion;
    if (!spec || (spec.const === undefined && spec.type === undefined))
        return null;
    if (path !== "")
        return null;
    if (!isObject(value))
...
function issue(kind, path, message) {
    return { kind, path: path || "/", message };
}
function joinFs(dir, file) {
    if (!dir)
        return file;
