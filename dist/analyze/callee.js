import { exportNameOf } from "./model.js";
import { specifierMatches } from "./reexports.js";
export function unwrapExpr(ts, expr) {
    let cur = expr;
    for (;;) {
...
export function resolveCallee(ts, expr, localSet, wanted) {
...
export function peelCallApplyBind(ts, expr, localSet, wanted) {
    expr = unwrapExpr(ts, expr);
    if (ts.isPropertyAccessExpression(expr) && ts.isIdentifier(expr.name)) {
...
export function thisOf(ts, expr) {
    expr = unwrapExpr(ts, expr);
    if (ts.isPropertyAccessExpression(expr) && ts.isIdentifier(expr.name)) {
        const n = expr.name.text;
        if (n === "call" || n === "apply" || n === "bind")
    ...
    if (ts.isCallExpression(expr) && ts.isPropertyAccessExpression(expr.expression)) {
        ...
        if (ts.isIdentifier(n) && n.text === "call")
            return { kind: "call" };
        if (ts.isIdentifier(n) && n.text === "apply")
            return { kind: "apply" };
        if (ts.isIdentifier(n) && n.text === "bind")
            return { kind: "bind" };
    }
    return { kind: "unbound" };
}
export function asBindingEscape(ts, arg, localSet) {
    if (ts.isSpreadElement(arg))
        return asBindingEscape(ts, arg.expression, localSet);
...
export function namespaceIdent(ts, expr, localSet) {
    const id = unwrapExpr(ts, expr);
    if (!ts.isIdentifier(id) || !localSet.has(id.text))
        return null;
    const b = localSet.get(id.text);
    return b.imported === "*" ? b : null;
}
function isRequireOfWanted(ts, expr, wanted) {
...
export function originCallSite(ts, expr, lookupIdent, callByNode) {
    const u = unwrapExpr(ts, expr);
    if (ts.isIdentifier(u))
        return lookupIdent(u.text);
    ...
    if (ts.isPropertyAccessExpression(u))
        return originCallSite(ts, u.expression, lookupIdent, callByNode);
