...
export function isDynamicCodeCallee(ts, expr, aliases) {
    const cur = unwrapDeep(ts, expr);
    if (ts.isIdentifier(cur)) {
        if (cur.text === "eval" || cur.text === "Function")
            return cur.text;
        return aliases.get(cur.text) ?? null;
    }
    if (ts.isPropertyAccessExpression(cur) && ts.isIdentifier(cur.name)) {
        if (!DYN_NAMES.has(cur.name.text) || !isGlobalObj(ts, cur.expression))
            return null;
    ...
    if (ts.isElementAccessExpression(cur)) {
        ...
        if (!arg || !ts.isStringLiteral(arg) || !DYN_NAMES.has(arg.text))
            return null;
        if (!isGlobalObj(ts, cur.expression))
            return null;
        return arg.text === "Function" ? "Function" : "eval";
    }
    return null;
}
export function bindingFromExpr(ts, expr, localSet) {
    expr = unwrapDeep(ts, expr);
    if (ts.isIdentifier(expr) && localSet.has(expr.text))
        return localSet.get(expr.text);
...
export function bindPatternOrIdent(ts, name, initializer, localSet, bindings) {
    if (!initializer)
        return;
    ...
    for (const stmt of sf.statements) {
        ...
        if (ts.isExportDeclaration(stmt) &&
            ...
            for (const el of stmt.exportClause.elements) {
                ...
                if (!b || !parseSpecifier(b.specifier))
                    continue;
                extra.pkgLinks.push({
...
export function pushUnknown(ts, sf, node, extra, unknowns, kind, detail, widensTo, prefix) {
    unknowns.push({
...
export function identifierValueEscape(ts, ident, localSet) {
    if (!localSet.has(ident.text))
        return null;
    if (isAccountedIdentifier(ts, ident))
        return null;
    return localSet.get(ident.text);
}
function isGlobalObj(ts, expr) {
    const obj = unwrapDeep(ts, expr);
    if (obj.kind === ts.SyntaxKind.ThisKeyword)
        return true;
    return ts.isIdentifier(obj) && GLOBAL_OBJS.has(obj.text);
}
function isAccountedIdentifier(ts, ident) {
    if (isTypeish(ts, ident))
        return true;
    const p = ident.parent;
    if (!p)
        return true;
    if (ts.isTypeQueryNode(p) || ts.isTypeNode(p))
        return true;
    ...
    if (ts.isImportSpecifier(p) || ts.isImportClause(p) || ts.isNamespaceImport(p))
        return true;
    if (ts.isExportSpecifier(p) || ts.isNamespaceExport(p))
        return true;
    if (ts.isBindingElement(p) || ts.isParameter(p))
        return true;
    if (ts.isVariableDeclaration(p) && p.name === ident)
        return true;
    if (ts.isPropertyAssignment(p) && p.name === ident)
        return true;
    ...
    if (ts.isElementAccessExpression(p) && unwrapDeep(ts, p.expression) === ident)
        return true;
    if ((ts.isCallExpression(p) || ts.isNewExpression(p)) && unwrapDeep(ts, p.expression) === ident) {
        return true;
    }
    if (ts.isTaggedTemplateExpression(p) && unwrapDeep(ts, p.tag) === ident)
        return true;
    if (ts.isVariableDeclaration(p) && p.initializer && identInExpr(ts, p.initializer, ident)) {
        if (ts.isIdentifier(p.name) || ts.isObjectBindingPattern(p.name))
            return true;
    }
...
function identInExpr(ts, expr, ident) {
    const cur = unwrapDeep(ts, expr);
    return cur === ident;
}
function isTypeish(ts, ident) {
