...
export function parseSpecifier(specifier) {
    ...
    if (specifier.startsWith("#"))
        return null;
    if (WINDOWS_ABS.test(specifier))
        return null;
    if (PROTOCOL.test(specifier))
        return null;
    if (isBuiltin(specifier) || isBuiltin(`node:${specifier}`))
        return null;
    if (specifier.startsWith("@")) {
    ...
    if (isBuiltin(name) || isBuiltin(`node:${name}`))
        return null;
    if (slash === -1)
        return { name, subpath: "" };
    return {
...
export function resolvePackageImports(specifier, importMap) {
    if (!specifier.startsWith("#"))
        return specifier;
    ...
    const target = typeof raw === "string"
        ? raw
        : raw && typeof raw === "object" && typeof raw.default === "string"
            ? raw.default
            : null;
    if (!target)
        return null;
...
export function resolvePackageFamily(specifier) {
...
export function installedVersion(deps, name) {
    const raw = deps?.[name] ?? "";
