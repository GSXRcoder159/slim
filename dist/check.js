...
import { EXIT_FAIL, EXIT_OK, SlimExit } from "./exit.js";
import { loadProject, loadTargetTypescript } from "./project.js";
import { loadConfig } from "./config.js";
import { analyzePackage } from "./analyze/index.js";
import { specifierMatches, wantedSpecifiers } from "./analyze/reexports.js";
import { withLocalBinPath } from "./replace.js";
...
import { diffEnvelope } from "./envelope/drift.js";
import { checkContracts } from "./generate/exports.js";
import { detectRunner } from "./trace/runners.js";
import { evidenceScript, hardeningTestPaths, hasStandingTests, standingTestPaths, } from "./evidence/paths.js";
import { replacementStateIssues, } from "./upstream/state.js";
export { evidenceScript, hardeningTestPaths, standingTestPaths } from "./evidence/paths.js";
function childText(chunk) {
    if (chunk == null)
        return "";
    return typeof chunk === "string" ? chunk : chunk.toString("utf8");
}
export const DEFAULT_CHECK_CHILD_TIMEOUT_MS = 600_000;
export function checkChildTimeoutMs(env = process.env) {
    const raw = env.SLIM_CHECK_CHILD_TIMEOUT_MS?.trim();
    if (!raw)
        return DEFAULT_CHECK_CHILD_TIMEOUT_MS;
    if (!/^[1-9][0-9]*$/.test(raw)) {
...
function childTimedOut(r) {
    const err = r.error;
    return err?.code === "ETIMEDOUT";
}
function spawnChecked(spawn, command, args, root, failMessage, json) {
...
export function runStandingTests(root, pkg, outDir, spawn = spawnSync, json = false) {
    ...
    if (existsSync(tsAbs)) {
        spawnChecked(spawn, process.execPath, ["--experimental-strip-types", "--test", tsRel], root, `standing tests failed for ${pkg}`, json);
        return;
    }
    if (existsSync(jsAbs)) {
...
export function runHardenedTests(root, moduleRel, spawn = spawnSync, json = false) {
    if (!moduleRel)
        return;
    ...
    if (existsSync(tsAbs)) {
        if (vitest) {
            ...
            spawnChecked(spawn, process.execPath, ["--experimental-strip-types", "--test", tsRel], root, `hardening tests failed for ${moduleRel}`, json);
        }
        return;
    }
    if (existsSync(jsAbs)) {
...
export function runConfiguredTestCommand(root, testCommand, spawn = spawnSync, json = false) {
    const cmd = testCommand?.trim() || null;
    if (!cmd)
        return;
    const parts = cmd.split(/\s+/).filter(Boolean);
    spawnChecked(spawn, parts[0], parts.slice(1), root, `testCommand failed: tests exited`, json);
}
function readEnvelope(path) {
    return readDocument("envelope", path, `envelope ${path}`);
}
function loadManifestRecords(root) {
    const manPath = join(root, ".slim", "manifest.json");
    if (!existsSync(manPath))
        return { recs: {}, error: null, missing: true };
    try {
        const json = readDocument("manifest", manPath, ".slim/manifest.json");
...
function originalPackageImports(root, files, pkg) {
...
function missingExports(root, moduleRel, saved) {
    if (!moduleRel)
        return [{ kind: "exports", detail: "missing slice module" }];
    const abs = join(root, moduleRel);
    if (!existsSync(abs))
        return [{ kind: "exports", detail: `missing slice module ${moduleRel}` }];
    let source;
    try {
    ...
    try {
        ...
        return result.errors.map((detail) => ({ kind: "exports", detail }));
    }
    catch {
        return [];
    }
}
function uniqDrift(items) {
    const seen = new Set();
    const out = [];
...
function emitDiag(args, line) {
    process.stderr.write(line.endsWith("\n") ? line : `${line}\n`);
}
export async function runCheck(args, opts = {}) {
    ...
    for (const pkg of names) {
        ...
        try {
            ...
            failed = true;
            if (!json && err instanceof SlimExit)
                throw err;
            continue;
        }
        ...
        if (drift.length) {
            emitDiag(args, `${pkg}: envelope drifted (${drift.map((d) => d.detail).join("; ")}). re-run slim replace ${pkg}`);
            failed = true;
        }
        else if (!json) {
    ...
    if (!names.length) {
        ...
        if (failed)
            throw new SlimExit(EXIT_FAIL, "slim check failed", { skipJson: json });
        return EXIT_OK;
    }
    try {
