const NO_TRACES = "no traces — generators are static-shape plus catalog mutations, not your runtime distribution";
export function closeEnvelope(env, opts) {
    ...
    const traceClosed = dynamic.length > 0 &&
    ...
    if (env.imports.length === 0 && env.symbols.length === 0) {
        ...
        if (ready)
            reason += "; --allow-unknown (unsafe override, not closed)";
    }
    else if (traceClosed) {
