...
export async function runDebounceScript(debounceFn, script, clock) {
    const spies = [];
    const returns = [];
    const flushResults = [];
    ...
    try {
        ...
        const spy = function spy(...args) {
            const ev = { t: clock.getTime(), thisArg: this, args };
            if (script.throwing) {
        ...
        const wrapper = (script.options === undefined
...
export function taxonomyForObserved(opts) {
    ...
    if (hasOptions) {
        ...
        const wantTrailingCombo = lits.some((o) => isRecord(o) && o.leading === true && o.trailing !== false);
        if (wantLeading)
            out.push(TAXONOMY["leading-only"]);
        if (wantTrailingCombo) {
            out.push(TAXONOMY["leading-trailing-one"], TAXONOMY["leading-trailing-two"]);
        }
        if (wantMax)
            out.push(TAXONOMY["maxWait-stream"]);
        if (!wantLeading && !wantMax && !wantTrailingCombo) {
            out.push(TAXONOMY["leading-only"], TAXONOMY["leading-trailing-one"], TAXONOMY["leading-trailing-two"], TAXONOMY["maxWait-stream"]);
        }
    }
    return out;
}
export function isTimerSymbol(name) {
    return /^(debounce|throttle)$/i.test(name);
}
function isRecord(v) {
    return v !== null && typeof v === "object" && !Array.isArray(v);
}
//# sourceMappingURL=debounce-driver.js.map