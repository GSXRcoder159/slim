 ...
 */
export function minimize(args, pred, deadlineMs) {
    const start = wallMs();
    const timedOut = () => wallMs() - start >= deadlineMs;
    let current = clone(args);
    if (!pred(current))
        return current;
...
function shrinkValue(value, still) {
    if (typeof value === "string" && value.length > 0) {
        let s = value;
        while (s.length > 1) {
            const half = s.slice(0, Math.ceil(s.length / 2));
            if (half !== s && still(half))
                s = half;
            else
                break;
        }
    ...
    if (value !== null && typeof value === "object" && !(value instanceof Date)) {
        ...
        if (Object.keys(o).length && still({}))
            return {};
        return o;
    }
    if (typeof value === "number" && value !== 0 && still(0))
        return 0;
    return value;
}
