...
import { createGen, fromTraces, mutateArgs, fromShapes, junkArgs, enumerateLiteralUnions, isExportTrace, pickObservedArgc, } from "./gen.js";
import { deserializeEvent } from "../trace/serialize.js";
import { extraCaseQuota, createFakeClock, wallMs } from "./clock.js";
import { taxonomyForObserved, isTimerSymbol, TAXONOMY, } from "./debounce-driver.js";
import { createPool, defaultJobTimeoutMs, loadOrig, loadSlim, runJob, withSlimQuery, toCloneableResult, fromCloneableResult, } from "./workers.js";
import { symbolMatches } from "../trace/attribute.js";
...
export function isInjectableCrypto(pkg) {
...
function assertFuzzAllowed(envelope, opts) {
    const pkg = envelope.package.name;
    if (envelope.cryptoRandom && !opts.allowFlaky && !isInjectableCrypto(pkg)) {
        throw new SlimExit(EXIT_REFUSED, `refused fuzz: unseeded RNG package ${pkg} without injectable isolation; pass --allow-flaky to override`);
    }
    const returnsNow = envelope.symbols.some((s) => s.exportName === "now");
    if (returnsNow && envelope.clock === false) {
        throw new SlimExit(EXIT_REFUSED, `refused fuzz: Date.now as a returned value (symbol now) requires clock isolation`);
    }
...
function assertRequiredFunctions(original, replacement, envelope) {
...
function cryptoSeedFor(envelope, allowFlaky, seed, caseIndex) {
    if (allowFlaky)
        return undefined;
    if (!envelope.cryptoRandom || !isInjectableCrypto(envelope.package.name))
        return undefined;
...
function planCases(opts) {
    ...
    for (const sym of timerSymbols) {
        for (const script of scriptsForSymbol(sym, opts.envelope)) {
            add({ symbol: sym.exportName, args: [script], kind: "debounce", script, hyrum: sym.hyrum }, { isDebounce: true, script });
        }
    }
    ...
    for (const sym of valueSymbols) {
        ...
        for (const tr of traces) {
            ...
            add({ symbol: sym.exportName, args: live.args, thisArg: live.thisArg, kind: "call", hyrum }, { isTrace: true });
        }
...
function applyPlanned(report, item, r) {
...
function throwIfEmpty(report) {
    if (report.cases === 0) {
        throw new SlimExit(EXIT_ENV, "insufficient budget");
    }
}
async function executePlanOnPool(pool, plan, report, workers, timeoutMs) {
    const slots = new Map();
    const running = new Map();
    let applyAt = 0;
    let issued = 0;
    const width = Math.max(1, workers);
    const start = (seq, item) => {
        const p = pool
...
function emptyReport(seed, allowFlaky) {
    return {
        cases: 0,
        comparisons: 0,
        timerCases: 0,
...
function recordPoolCall(report, r) {
...
function recordPoolDebounce(report, r, script) {
    if (r.reason?.startsWith("missing function")) {
        throw new SlimExit(EXIT_FAIL, r.reason);
    }
    report.cases++;
...
export function traceHits(tr, sym) {
    if (symbolMatches(sym.exportName, tr.symbol))
        return true;
...
function closedArgShapes(sym) {
    if (!sym.callSites.length)
        return false;
    return sym.callSites.every((c) => c.argShapes.length > 0 &&
        c.argShapes.every((s) => s.kind === "literal" || s.kind === "date" || s.kind === "union"));
}
function pickFuzzArgs(gen, replayed, sym) {
    const closed = closedArgShapes(sym) ||
        (replayed.length > 0 &&
            replayed.every((args) => args.every((a) => a instanceof Date || typeof a === "string" || typeof a === "number")));
    const r = gen.next();
    if (replayed.length && (closed || r < 0.7)) {
        const base = gen.pick(replayed);
        if (!closed && r < 0.35)
            return mutateArgs(base, gen);
        return base.slice();
    }
    ...
    const argc = pickObservedArgc(sym.callSites.flatMap((c) => c.argc.observed), gen, 2);
    return junkArgs(argc, gen);
}
function exportTracesFor(envelope, sym) {
    return envelope.traces.filter((tr) => traceHits(tr, sym) && isExportTrace(tr));
}
function scriptsForSymbol(sym, envelope) {
    ...
    for (const c of sym.callSites) {
        ...
        if (opt?.literals)
            optionLiterals.push(...opt.literals);
    }
