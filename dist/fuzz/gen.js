import { deserialize, deserializeEvent } from "../trace/serialize.js";
import { clone } from "./clone.js";
/** mulberry32 — deterministic, 32-bit. */
export function createGen(seed) {
    let s = seed >>> 0;
    const next = () => {
        s = (s + 0x6d2b79f5) >>> 0;
        let t = s;
    ...
    return {
        ...
        pick(xs) {
            if (xs.length === 0)
                throw new Error("Gen.pick: empty array");
...
export function mutateArgs(args, gen) {
    ...
    switch (kind) {
        case 0: {
            ...
            if (obj)
                obj[`extra_${gen.int(0, 99)}`] = gen.pick([1, "x", true, null]);
            else
                out.push({ extra: true });
            break;
        }
        ...
        case 6:
            if (out.length)
                out[gen.int(0, out.length - 1)] = null;
            else
                out.push(null);
            break;
        case 7:
            if (out.length)
                out[gen.int(0, out.length - 1)] = undefined;
            else
                out.push(undefined);
            break;
        case 8: {
        ...
        case 9:
        default: {
            if (gen.bool() && out.length)
                out.pop();
            else
                out.push(junkValue(gen));
            break;
        }
    }
    return out;
}
export function fromShapes(shapes, gen, argc = shapes.length) {
...
export function isExportTrace(tr) {
    return tr.parentOriginId === undefined && tr.resultMember === undefined;
}
export function pickObservedArgc(observed, gen, fallback = 2) {
    if (!observed.length)
        return fallback;
    return gen.pick(observed);
}
export function junkArgs(argc, gen) {
    const n = Math.max(0, argc);
    const out = [];
    for (let i = 0; i < n; i++)
        out.push(junkValue(gen));
...
export function enumerateLiteralUnions(shapes, cap) {
    if (!shapes.length)
        return [];
    if (!shapes.every((s) => (s.kind === "literal" || s.kind === "union") && s.literals && s.literals.length)) {
        return [];
    }
    const domains = shapes.map((s) => s.literals ?? []);
    const combos = cartesian(domains, cap);
    return combos;
}
function fromShape(shape, gen, depth) {
    if (depth > 6)
        return null;
    switch (shape.kind) {
        case "literal":
        case "union":
            if (shape.literals && shape.literals.length)
                return gen.pick(shape.literals);
        ...
        case "array": {
            ...
            const n = gen.int(0, 3);
            const inner = shape.props
                ? { kind: "object", props: shape.props }
                : { kind: "any" };
            const a = [];
            for (let i = 0; i < n; i++)
                a.push(fromShape(inner, gen, depth + 1));
            return a;
        }
        case "function": {
...
export function hydrate(value, _refs) {
    return deserialize(value);
}
function junkValue(gen) {
...
function indexOfType(args, t) {
    return args.findIndex((a) => typeof a === t);
}
function cartesian(domains, cap) {
    let acc = [[]];
