 ...
 */
export function cmdShim(name) {
    if (process.platform !== "win32")
        return name;
    if (/\.(cmd|bat|exe)$/i.test(name))
        return name;
    if (name === "bun" || name === "node" || name === "git" || name === "tar")
        return name;
    return `${name}.cmd`;
}
export function cmdShimSpawnOpts(bin) {
...
export function scriptSpawnOpts(file) {
    if (process.platform !== "win32")
        return {};
    if (file === process.execPath || /\.exe$/i.test(file))
        return {};
    return { shell: true, windowsHide: true };
}
export function spawnPm(name, args, opts = {}) {
...
export function resolveScriptFile(file) {
    if (file === "node" || file === "node.exe")
        return process.execPath;
    return cmdShim(file);
}
export function execPm(name, args, opts = {}) {
    const bin = cmdShim(name);
...
export function hermeticPmEnv(extra = {}) {
...
function pmBinary(lockfile) {
    return cmdShim(pmName(lockfile));
}
export function installCommandFor(lockfile) {
    return `${pmName(lockfile)} install`;
}
export function refreshLockfile(project, opts, execFile = execFileSync) {
    if (opts && !shouldRefreshLockfile(opts))
        return;
...
function frozenInstallArgs(bin, env, frozen) {
    const kind = bin.replace(/\.cmd$/i, "");
    if (kind === "pnpm") {
        const args = ["install"];
        if (env.npm_config_store_dir)
            args.push("--store-dir", env.npm_config_store_dir);
