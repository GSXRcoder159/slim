import { parseSpecifier, resolvePackageFamily } from "../analyze/family.js";
/** Specifiers to rewrite: observed import sites plus the package being replaced. */
export function rewriteSpecifiers(env, pkg) {
    const specs = new Set();
    specs.add(pkg);
    specs.add(env.package.name);
    for (const i of env.imports)
        specs.add(i.specifier);
    return specs;
}
function declaredName(dep) {
 ...
 */
export function removeDependencyNames(env, declared) {
    const importedNames = new Set();
    for (const i of env.imports) {
        const parsed = parseSpecifier(i.specifier);
        if (parsed)
            importedNames.add(parsed.name);
        else
            importedNames.add(i.specifier);
    }
    const out = new Set([env.package.name]);
    const family = env.package.family;
    for (const dep of declared) {
        const name = declaredName(dep);
        const fam = resolvePackageFamily(name);
        if (!fam || fam.family !== family)
            continue;
