...
function confined(rootReal, target) {
    return target === rootReal || target.startsWith(rootReal + sep);
}
export function dirSize(path, capFiles = 4000) {
    let total = 0;
    let n = 0;
    let complete = true;
    let reason = "";
...
export function estimatePackageSize(projectRoot, name, opts = {}) {
...
export function gzipGuess(minBytes) {
    return Math.round(minBytes * 0.36);
}
export function readInstalledVersion(projectRoot, name) {
