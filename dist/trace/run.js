...
import { mergeTraces } from "../envelope/merge.js";
import { resolvePackageFamily } from "../analyze/family.js";
import { detectRunner, writeVitestTraceConfig, buildTraceSpawn, traceEnv, } from "./runners.js";
import { isErrorRecord, isSessionRecord } from "./session.js";
export const TRACE_TIMEOUT_MS = 120_000;
export const MAX_TRACE_BYTES = 32 * 1024 * 1024;
export const MAX_TRACE_EVENTS = 50_000;
export function writeTracesMeta(pkgDir) {
...
export function withLocalBinPath(root, env = process.env) {
    const bin = join(root, "node_modules", ".bin");
    const out = { ...env };
    writeEnvPath(out, `${bin}${delimiter}${readEnvPath(out)}`);
    for (const k of HOST_TEST_ENV)
        delete out[k];
    return out;
}
export function runTraces(root, pkg, env, opts) {
    const runner = detectRunner(root);
    if (runner.kind === "jest") {
        if (runner.jestSnippet)
            process.stderr.write(runner.jestSnippet + "\n");
        throw new SlimExit(EXIT_ENV, "Jest is detect-only; Slim does not wrap Jest. Use node:test (--import slim/hooks) or Vitest (slim/vitest), or pass --no-trace for static-only evidence (cannot claim trace closure).");
    }
    if (runner.kind === "none" || !runner.command) {
        throw new SlimExit(EXIT_ENV, "no test runner for traces. Add a node:test or Vitest script, or pass --no-trace for static-only evidence (cannot claim trace closure).");
    }
    ...
    try {
        ...
        throw new SlimExit(EXIT_ENV, `trace hook missing: ${err instanceof Error ? err.message : String(err)}`);
    }
    const fam = resolvePackageFamily(pkg);
    const packages = [pkg, env.package.name, fam?.name, fam?.family].filter(Boolean);
    if (fam?.family === "lodash")
        packages.push("lodash", "lodash-es");
    const uniq = [...new Set(packages)];
    const envVars = {
        ...traceEnv(uniq, outPath),
        SLIM_TRACE_ROOT: root,
    };
    const vitestConfigPath = runner.kind === "vitest" ? writeVitestTraceConfig(root, uniq) : undefined;
    ...
    if (r.status !== 0) {
        throw new SlimExit(EXIT_FAIL, `trace run exited ${r.status ?? "null"}\n${[(r.stderr ?? "").trim(), (r.stdout ?? "").trim()].filter(Boolean).join("\n").slice(0, 800)}`);
    }
    const parsed = readTraceFile(outPath);
    if (!parsed.sawSession) {
        const detail = [(r.stderr ?? "").trim(), (r.stdout ?? "").trim()].filter(Boolean).join("\n").slice(0, 800);
        throw new SlimExit(EXIT_ENV, `trace hook did not load (missing session header). Check slim/hooks or slim/vitest resolution.${detail ? `\n${detail}` : ""}`);
    }
    if (parsed.errors.length) {
        const first = parsed.errors[0];
        throw new SlimExit(EXIT_FAIL, `trace ${first.kind}${first.message ? `: ${first.message}` : ""}`);
    }
    if (!parsed.events.length) {
        throw new SlimExit(EXIT_FAIL, "zero package events; runtime not observed. Pass --no-trace for static-only evidence (cannot claim trace closure).");
    }
    const merged = mergeTraces(env, parsed.events, { root });
    if (merged.traces.some((t) => t.unmatched)) {
        throw new SlimExit(EXIT_FAIL, "unmatched trace events; cannot attribute runtime observations to static call sites");
    }
    return merged;
}
