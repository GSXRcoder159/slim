...
export function extractEsmExportNames(source, opts) {
    const names = new Set();
    const seen = opts?.seen ?? new Set();
    if (opts?.parentUrl) {
        if (seen.has(opts.parentUrl))
            return [];
        seen.add(opts.parentUrl);
    }
    if (/\bexport\s+default\b/.test(source))
        names.add("default");
    for (const m of source.matchAll(/\bexport\s+(?:async\s+)?(?:function\*?|class)\s+([A-Za-z_$][\w$]*)/g)) {
        if (m[1])
            names.add(m[1]);
    }
    ...
    for (const m of source.matchAll(/\bexport\s+\*\s+as\s+([A-Za-z_$][\w$]*)\s+from\s+["']([^"']+)["']/g)) {
        if (m[1])
            names.add(m[1]);
    }
    for (const m of source.matchAll(/\bexport\s+\*\s+from\s+["']([^"']+)["']/g)) {
        ...
        for (const n of extractCjsExportNames(child.source))
            names.add(n);
    }
    return [...names];
}
export function extractCjsExportNames(source) {
    const names = new Set();
...
function resolveStarTarget(spec, opts) {
    if (!opts?.parentUrl)
        return null;
...
function readModule(url, opts) {
    if (opts?.read)
        return opts.read(url);
    try {
...
//# sourceMappingURL=esm-names.js.map