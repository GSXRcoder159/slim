...
export function sourceOk<T>(value: T, detail = "ok"): SourceResult<T> {
  return { status: "success", value, detail };
}

export function sourceErr<T>(
  status: Exclude<SourceStatus, "success">,
  detail: string,
): SourceResult<T> {
  return { status, detail };
}

export function sourceNotRequired<T>(): SourceResult<T> {
  return { status: "success", detail: "not required" };
}

export function isConsultedFailure(s: { status: SourceStatus; detail: string }): boolean {
...
export async function fetchJson(
  ...
  try {
    return sourceOk(JSON.parse(text) as unknown);
  } catch {
