...
function defaultExec(
  ...
  return String(
    execFileSync(file, [...args], { ...opts, stdio: ["ignore", "pipe", "pipe"] }),
  );
}

export function findBundleEntry(root: string): string | null {
  if (existsSync(join(root, "wrangler.toml"))) {
    const text = readFileSync(join(root, "wrangler.toml"), "utf8");
    const m = text.match(/^\s*main\s*=\s*["']([^"']+)["']/m);
    if (m?.[1] && existsSync(join(root, m[1]))) return m[1];
  }
...
export function maybeBundleBytes(root: string, deps: BundleDeps = {}): BundleDelta | null {
  ...
  if (hasBin("wrangler")) {
    ...
    try {
      ...
      if (bytes > 0) return { tool: "wrangler", bytes, entry };
    } catch {
  ...
  if (hasBin("esbuild")) {
    ...
    try {
      execFile(
        "esbuild",
        [entry, "--bundle", "--outfile=" + out, "--platform=neutral", "--format=esm", "--minify"],
        { cwd: root, encoding: "utf8", timeout: 30_000 },
      );
      if (existsSync(out)) return { tool: "esbuild", bytes: readFileSync(out).byteLength, entry };
    } catch {
      return null;
    } finally {
      try {
