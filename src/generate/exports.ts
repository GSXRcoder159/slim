...
import type { Envelope, ImportKind } from "../envelope/types.ts";

const IGNORED = new Set(["*", "(scan)"]);
const DEFAULT_KINDS: ImportKind[] = ["default", "namespace", "cjs-require", "subpath-default"];
...
export function checkContracts(
  ...
  for (const name of required) {
    if (!named.has(name)) errors.push(`missing named export ${name}`);
  }
  ...
  for (const s of envelope.symbols) {
    for (const m of s.resultMembers) members.add(m);
    for (const c of s.callSites) {
      for (const m of c.resultMembers) members.add(m);
    }
  }
  for (const m of members) {
    if (!valueProps.has(m)) errors.push(`missing result member ${m}`);
  }
...
function collectExports(
  ...
  if (ts.isFunctionDeclaration(node) || ts.isClassDeclaration(node) || ts.isEnumDeclaration(node)) {
    ...
    if (mods.some((m) => m.kind === ts.SyntaxKind.DefaultKeyword)) onDefault();
    return;
  }
  ...
  if (ts.isExportDeclaration(node) && node.exportClause && ts.isNamedExports(node.exportClause)) {
    for (const el of node.exportClause.elements) {
      ...
      if (el.name.text === "default" || el.propertyName?.text === "default") onDefault();
    }
  }
}

function addObjectKeys(ts: typeof import("typescript"), expr: ts.Expression, keys: Set<string>): void {
...
function collectValueProps(ts: typeof import("typescript"), node: ts.Node, props: Set<string>): void {
  ...
  if (ts.isShorthandPropertyAssignment(node)) props.add(node.name.text);
}
