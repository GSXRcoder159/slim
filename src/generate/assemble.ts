  ...
  const visit = (file: string) => {
    ...
    for (const stmt of sf.statements) {
      if (
        ...
        if (!spec.startsWith("./") || INTERNAL_SPEC.test(spec)) continue;
        const resolved = firstExisting(
          join(dirname(file), spec),
          join(dirname(file), spec.replace(/\.js$/, ".ts")),
...
function importedInternalNames(ts: typeof import("typescript"), src: string): string[] {
  ...
  for (const stmt of sf.statements) {
    ...
    if (!INTERNAL_SPEC.test(stmt.moduleSpecifier.text)) continue;
...
function unexport(text: string): string {
