import type { ArgShape, CallSite, Envelope, ImportKind } from "./types.ts";
...
function isTrackedSymbol(name: string | undefined): boolean {
  return Boolean(name) && !SKIP_SYMBOLS.has(name!);
}

function canonShape(shape: ArgShape | undefined): unknown {
...
export function callSiteFingerprint(site: CallSite): string {
...
function symbolNames(env: Envelope): string[] {
...
function resultMembers(env: Envelope, exportName: string): Set<string> {
  const out = new Set<string>();
  for (const s of env.symbols ?? []) {
    if (s.exportName !== exportName) continue;
...
function importKinds(env: Envelope): Set<ImportKind> {
  return new Set((env.imports ?? []).map((i) => i.kind));
}

function unknownKeys(env: Envelope): Set<string> {
  return new Set((env.unknowns ?? []).map((u) => `${u.kind}:${u.id}`));
}

export function diffEnvelope(saved: Envelope, live: Envelope): EnvelopeDrift[] {
